...
//     doc.setFontSize(11);
//     doc.text(`TOTAL: ${quotation.total}`, rightXTotal, finalY);
//     doc.text(`DISCOUNT: ${quotation.discount}`, rightXTotal, finalY + 16);
//     doc.text(`BALANCE: ${balance}`, rightXTotal, finalY + 32);
//     doc.text(`GRAND TOTAL: ${quotation.grandTotal}`, rightXTotal, finalY + 48);
...
export const generateInvoicePDF = async (quotationId: string) => {
  try {
    ...
    // ✅ Map display name with new formats
    const getDisplayItem = (itemName: string) => {
      const invItem = inventoryItems.find((inv: any) => inv.name === itemName);
      if (invItem) {
